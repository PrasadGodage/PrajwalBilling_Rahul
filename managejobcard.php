      ...
      <table class="table table-striped table-hover" id="tableExport" style="width:100%;">
        <thead>
          <tr>
            ...
            <th>Jobcard Lot.No</th>
            <th>Jobcard Inv.No</th>
          </tr>
        </thead>
        <tbody>
<?php

$selectmenulist="SELECT * FROM `jobcardmaster`";
  ...
  $num=1;
    ...
    {?>
        <tr>
          ...
          if($row['lotno']=="")
          {
            echo "<button class='mt-1 btn btn-primary' onclick=openlotmodel('".$row['jobcardid']."')>Add LOT.No</button>";
          }else
          {
            echo $row['lotno'];
          }
          ...
          if($row['jbcinvno']=="")
          {            
            // data-toggle='modal' data-target='#modelforinvoicenoadd'
            echo "<button class='mt-1 btn btn-primary'  onclick=openinvmodel('".$row['jobcardid']."')>Add invoice no</button>";
          }else
          {
            echo $row['jbcinvno'];
          }
