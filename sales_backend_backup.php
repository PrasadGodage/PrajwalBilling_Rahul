	 ...
	 isset($_POST['SgstPercentage'])&& isset($_POST['SgstAmt']) && isset($_POST['TaxAmount']) && isset($_POST['NetAmount']) )
{
    ...
    if(isset($_POST['TEMPBillNo']) && isset($_POST['BillDate']) && isset($_POST['customerid']) && isset($_POST['paymentType']) && 
     ...
     isset($_POST['NetBillTotal'])&& isset($_POST['BillCharges']) && isset($_POST['ExtraBilldiscount'])&&
	 isset($_POST['BillAmountForPaid'])&& isset($_POST['PaidAmount']) && isset($_POST['BalanceAmount']))
{
	$totalQty="0";
		...
		if(isset($_POST['readrecord']) && isset($_POST['BillNO']))
		{
					...
					while($row=mysqli_fetch_array($res))
			{
				$output.="<tr>
					...
					<td>".$row['NetAmt']."</td>                                         
					<td>
						<button class='mt-1 btn btn-danger' onclick='deletedata(".$row['ItemId'].")'>X</button>
					</td>
					</tr>";
				// <?php 
				$num++;
